...
{/* 
            ...
            {[
              ...
              { icon: <FlaskConical size={18} />, label: "Botanical Active Blend" },
            ].map((item) => (
              <div
                key={item.label}
...
function VariantGrid({ products }: { products: Product[] }) {
  ...
  <div key={product._id} className="flex flex-col gap-2">
    {/* Variant label above card — always reserve this space so cards align */}
    <div className="flex flex-col gap-1 px-1 min-h-[2.5rem]">
      {product.variantLabel ? (
        ...
        // invisible placeholder keeps the same height as cards that DO have a label
...
function TrustStrip() {
  const items = [
    { icon: <Leaf size={22} />, label: "Natural Active Ingredients",     sub: "Carefully Selected Botanicals"     },
    { icon: <ShieldCheck size={22} />, label: "Trusted Quality", sub: "Made With Precision"     },
    { icon: <FlaskConical size={22} />, label: "Every Day Skin Care", sub: "Suitable for Daily Use" },
    { icon: <Star size={22} />, label: "Made in India",          sub: "Crafted with Care"  },
  ]
  return (
